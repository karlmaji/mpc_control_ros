...
class MPC_Control
{
    ...
    public:
        ...
        //设定 动态转移矩阵A B
        void setDynamicsMatrices(Eigen::Matrix<double, 3, 3> &a, Eigen::Matrix<double, 3, 2> &b,const Eigen::Matrix<double,3,1> &x0);
        //设定P Hession
        ...
        //求解
        bool step(const Eigen::Matrix<double,3,1>&x0,const std::vector<Eigen::Matrix<double,3,1>> &xref,const std::vector<Eigen::Matrix<double,2,1>> &uref);
