...
void MPC_Control::setDynamicsMatrices(Eigen::Matrix<double, 3, 3> &a, Eigen::Matrix<double, 3, 2> &b,const Eigen::Matrix<double,3,1> &x0)
{
    a<< 1.,0.,0.,
        0.,1.,0.,
        0.,0.,1.;
                                   ...
                                   int mpcWindow, Eigen::VectorXd &lowerBound, Eigen::VectorXd &upperBound)
{
    ...
    for(int i=0; i<mpcWindow+1; i++){
        lowerInequality.block(3*i,0,3,1) = xMin;
        upperInequality.block(3*i,0,3,1) = xMax;
    }
    for(int i=0; i<mpcWindow; i++){
    ...
    lowerEquality.block(0,0,3,1) = -x0;
...
bool MPC_Control::step(const Eigen::Matrix<double,3,1>&x0,const std::vector<Eigen::Matrix<double,3,1>> &xref,
                         const std::vector<Eigen::Matrix<double,2,1>> &uref)
{   
    this->setDynamicsMatrices(this->_a,this->_b,x0);
    this->castMPCToQPConstraintMatrix(this->_a,this->_b,this->_mpc_window,this->_linearMatrix);
