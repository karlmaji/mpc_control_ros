...
void DynaPlaning::DynamicProgramming()
{
    ...
    //std::cout<<origin.s<<' '<<origin.l<<std::endl;
    ...
    {
      if((all_obs+i)->l<(road_lu+2)&&(all_obs+i)->l>(road_ld-2)&&((all_obs+i)->s-origin.s)<plan_s_local)
      {
...
void DynaPlaning::AddPoint(int* best_index,double* para,Sample* dp_point,Frenet origin)
{
   ...
   { 
        ...
        {
            s=j*plan_s/60;
            this->dynamic_point[i*int(sample_s)+j].s=origin.s+i*sample_s+s;
        ...
        {
            s=j*plan_s/240;
            this->dynamic_point_f[i*int(sample_s)*4+j].s=origin.s+i*sample_s+s;;
